...
import aiofiles
import aiohttp
from MIA.converter import convert
import ffmpeg
import requests
from MIA.fonts import CHAT_TITLE
from PIL import Image, ImageDraw, ImageFont
from config import ASSISTANT_NAME, BOT_USERNAME, IMG_1, IMG_2, IMG_5, UPDATES_CHANNEL, GROUP_SUPPORT
from MIA.filters import command, other_filters
from MIA.queues import QUEUE, add_to_queue
from MIA.main import call_py, Test as user
from MIA.utils import bash
from MIA.main import bot as Client
...
async def generate_cover(thumbnail, title, userid, ctitle):
    ...
    image2 = Image.open("Process/ImageFont/grey.png")
...
async def play(c: Client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            ...
            if chat_id in QUEUE:
             ...
             try:
                ...
                await m.reply_photo(
                    photo=f"{IMG_2}",
                    caption=f"❄️ **ɴᴀᴍᴇ:** [{songname}]({link})\n💭 **ᴄʜᴀᴛ:** `{chat_id}`\n💡 **sᴛᴀᴛᴜs:** `ᴘʟᴀʏɪɴɢ`\n💘 **ʀᴇǫᴜᴇsᴛ ʙʏ:** {requester}\n📹 **sᴛʀᴇᴀᴍ ᴛʏᴘᴇ:** `ᴍᴜsɪᴄ`",
                    reply_markup=keyboard,
                )
             except Exception as e:
                await suhu.delete()
        ...
        f"**» ᴅᴏᴡɴʟᴏᴀᴅɪɴɢ ᴘʟᴇᴀꜱᴇ ᴡᴀɪᴛ ʙᴀʙʏ**"
    )
                ...
                if abhi == 0:
                    ...
                    if chat_id in QUEUE:
                        ...
                        try:
                            ...
                            await m.reply_photo(
                                photo=image,
                                caption=f"❄️ **ɴᴀᴍᴇ:** [{songname[:22]}]({url})\n**⏱ ᴅᴜʀᴀᴛɪᴏɴ:** `{duration}`\n💡 **sᴛᴀᴛᴜs:** `ᴘʟᴀʏɪɴɢ`\n💘 **ʀᴇǫᴜᴇsᴛ ʙʏ:** {requester}",
                                reply_markup=keyboard,
                            )
